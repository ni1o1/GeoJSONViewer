...
function wgs84tobd09(lon, lat) {
  let coord = wgs84togcj02(lon, lat)
  const gcj02lon = coord[0]
  const gcj02lat = coord[1]
  const bd09coord = gcj02tobd09(gcj02lon, gcj02lat)
  return bd09coord
}

function bd09towgs84_all(bd_lon, bd_lat) {
  let coord = bd09togcj02(bd_lon, bd_lat)
  const gcj02lon = coord[0]
  const gcj02lat = coord[1]
  const bd09coord = [bd_lon, bd_lat]
...
// 将 GeoJSON 对象的坐标从 BD-09 转换为 WGS-84
function convert_geojson(geojson, method) {
  turf.geomEach(geojson, function (currentGeometry, featureIndex, featureProperties, featureBBox, featureId) {
    if (currentGeometry.type === 'Point') {
      const [bd09_lng, bd09_lat] = currentGeometry.coordinates;
      const [wgs84_lng, wgs84_lat] = method(bd09_lng, bd09_lat);
      currentGeometry.coordinates = [wgs84_lng, wgs84_lat];
    } else if (currentGeometry.type === 'LineString' || currentGeometry.type === 'MultiPoint') {
      currentGeometry.coordinates = currentGeometry.coordinates.map(([bd09_lng, bd09_lat]) => {
        const [wgs84_lng, wgs84_lat] = method(bd09_lng, bd09_lat);
        return [wgs84_lng, wgs84_lat];
      });
    } else if (currentGeometry.type === 'Polygon' || currentGeometry.type === 'MultiLineString') {
      currentGeometry.coordinates = currentGeometry.coordinates.map(ring =>
        ring.map(([bd09_lng, bd09_lat]) => {
          const [wgs84_lng, wgs84_lat] = method(bd09_lng, bd09_lat);
          return [wgs84_lng, wgs84_lat];
        })
      );
    } else if (currentGeometry.type === 'MultiPolygon') {
      currentGeometry.coordinates = currentGeometry.coordinates.map(polygon =>
        polygon.map(ring =>
          ring.map(([bd09_lng, bd09_lat]) => {
            const [wgs84_lng, wgs84_lat] = method(bd09_lng, bd09_lat);
            return [wgs84_lng, wgs84_lat];
...
export { bd09towgs84, bd09mctobd09, wgs84togcj02,gcj02tobd09,wgs84tobd09, convert_geojson, bd09towgs84_all }